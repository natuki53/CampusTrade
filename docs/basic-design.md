...
| マイページ | 自分の出品履歴、購入履歴、取引中の商品を確認する。 |
...
| 商品詳細 | 商品情報、商品画像、出品者ニックネーム、購入ボタン、コメント、取引メッセージを表示する。 |
...
| 種類 | 用途 | 表示範囲 |
|---|---|---|
| 商品コメント | 購入前の質問や補足説明 | 商品詳細を閲覧できるユーザー全員 |
| 取引メッセージ | 購入後の受け渡し調整 | 出品者、購入者、管理者のみ |
...
| 表示範囲 | `message_type` により公開範囲を切り替える。 |
| 並び順 | 送信日時の昇順で表示する。 |
...
| 商品詳細画面 | `/products/{id}` | 不要 | 商品情報、購入ボタン、メッセージを表示する。 |
...
| マイページ | `/mypage` | 必要 | 出品履歴、購入履歴、取引中商品を表示する。 |
